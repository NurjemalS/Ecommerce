...
$userData = $_SESSION["user"] ;
  ...
  if( !validSession()) {
	  header("Location: consumerLogin.php?error") ;
	  exit ; 
  }
	  ...
	  if(empty($password) || empty($confirm_password)){
		...
		echo "<p class='error'> Do not match </p>";
		  ...
		  if(preg_match($reg_pass, $password) === 0){
			...
			$stmt = $db->prepare("UPDATE market_user SET m_password = :m_password WHERE m_email = :m_email") ;
			$stmt->execute([ "m_email" => $userData["m_email"], "m_password" => $hashPass]) ;
			header("Location: ../market/marketLogout.php") ;
			exit ;
		  }
