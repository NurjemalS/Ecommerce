      ...
      if ( checkUser($email, $pass) ) {
        ...
        if( $isExist == 0){
            if(empty($email) || empty($pass) || empty($name) ||  empty($city) ||  empty($address) ||  empty($district)){
                ...
                //$sql = "INSERT INTO market_user (m_email, m_name, m_city, m_district, m_address, m_password) values (?,?,?,?,?,?)";
                //$stmt = $db->prepare($sql) ;
                ...
                Mail::send($_SESSION["email"], "Authenticate your account", "Dear MARKET USER,<br> Vefification Code is  <b>" .$_SESSION["code"] . "</b> to authenticate your account.<br><br>Best wishes,<br>CTIS256 ") ;
                header("Location: confirmMarket.php") ;
                exit ;
            }
